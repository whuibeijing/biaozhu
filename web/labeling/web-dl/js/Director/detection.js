...
//线对象定义
function lineShape(x0,y0,type,score=1.0){
	this.type = type;
...
function copyOneBox(){
  ...
  for (var i=0;i<lineShapes.length;i++){
      if (lineShapes[i].isSelected==true){
        copyLineShapes.push(lineShapes[i]);
      }
  }
  currentImage=true;
}
...
function copy(){
  ...
  // copyPointShapes=[];
    ...
    console.log("拷贝数据："+JSON.stringify(copyPointShapes));
    currentImage=true;
}
...
function paste(){
	...
	if(!isEmpty(copyPointShapes)){
		...
		for(var i = 0; i <tmpPointShapes.length; i++){
          ...
          tmpPointShapes[i].isSelected=false;
		  pointShapes.push(tmpPointShapes[i]);
		}
	}
	if(!isEmpty(copyLineShapes)){
		var tmpLineShapes = copyLineShapes.slice();
		for(var i = 0; i <tmpLineShapes.length; i++){
          if (maxIdNum< parseInt(tmpLineShapes[i].other["region_attributes"]["id"])){ //不会出现重复的框
             maxIdNum = parseInt(tmpLineShapes[i].other["region_attributes"]["id"]);
          }
          else{
             tmpLineShapes[i].other["region_attributes"]["id"] = maxIdNum+1; //会出现重复的目标框
             tmpLineShapes[i].id = maxIdNum+1;
             maxIdNum = maxIdNum +1;
          }
          tmpLineShapes[i].isSelected=false;
		  lineShapes.push(tmpLineShapes[i]);
		}
	}
   drawRect();
   updateLabelHtml();
...
function endCreateLine(){
	if(!isEmpty(creatingLineShape)){
		creatingLineShape.finish = true;
	}
	creatingLineShape = null;
	toCreateLine = false;
}
...
function deleterect(){//删除矩形框或者mask
  ...
  if(SelectedLineShape){
	var index = lineShapes.indexOf(SelectedLineShape);
    lineShapes.splice(index, 1);
...
function drawPointShape(){
	for(var i = 0; i < pointShapes.length; i++){
		...
		//console.log("pointShapes=" + JSON.stringify(pointShapes));
...
function selectColor(tmpRegions,i){
	//console.log("regions=" + JSON.stringify(tmpRegions));
	if(!isEmpty(_via_attributes['region']['type'])){
		var dict = _via_attributes['region']['type']["options"];
		var color_num=0;
		//console.log("regions[i].other.id=" + JSON.stringify(tmpRegions[i]));
		for (var key in dict){
			if (key==tmpRegions[i].other.region_attributes.type){
...
function drawLineShape(x,y){
	for (var i=0; i<lineShapes.length; i++){
		  ...
		  selectColor(lineShapes,i);

		  for (var j=1; j<line.points.length; j++){
			context.beginPath();
			context.moveTo(bigLocX(line.points[j-1].x), bigLocY(line.points[j-1].y));
			context.lineTo(bigLocX(line.points[j].x),bigLocY(line.points[j].y));
			context.stroke();
			// context.closePath();
		  }
		  if(line.finish){
			...
			if(x&&y){
			  context.beginPath();
			  context.moveTo(bigLocX(line.points[mask.points.length-1].x),bigLocY(line.points[line.points.length-1].y));
			  context.lineTo(x,y);
			  context.stroke();
			  // context.closePath();              
			}
...
function canvasClick(e) {
	 ...
	 if(toCreateLine){
		    showpopVar = true;
    		hidePopup();
            if (creatingLineShape == null){
			  var lineShapeObj =new lineShape(smallClickX,smallClickY,boxcls);
			  lineShapes.push(lineShapeObj);
			  SelectedLineShape = lineShapeObj;
			  creatingLineShape = lineShapeObj;
            }
            else{
               creatingLineShape.points.push(new point(smallClickX,smallClickY))
           }
	 }
     //将选择信息设置为空，在下面重新查找设置
     resetSelectShapeEmpty();
...
function parse_labelinfo(labelinfo){
	...
	if(!isEmpty(labelinfo)){
		...
		for(var i=0;i<label_arr.length;i++){
		 ...
		 else if(!isEmpty(label_arr[i].line)){
		    cls=label_arr[i].class_name;
		    score = label_arr[i].score;
			
			var lineShapeObj = new lineShape(getCanvasLocationX(label_arr[i].line[0]),getCanvasLocationY(label_arr[i].line[1]),cls,score);
			
			for(var j = 2; j < label_arr[i].line.length; j+=2){
				  lineShapeObj.points.push(new point(getCanvasLocationX(label_arr[i].line[j]),getCanvasLocationY(label_arr[i].line[j+1])));
			}
			
			if(!isEmpty(label_arr[i].other)){
			   lineShapeObj.other = label_arr[i].other;
			}
			else if (isEmpty(label_arr[i].other)){
				lineShapeObj.other["region_attributes"]["type"] = pointShapeObj.type;
				lineShapeObj.other["region_attributes"]["id"] = pointShapeObj.id;
			}

		   if (maxIdNum< parseInt(lineShapeObj.other["region_attributes"]["id"])){
			  maxIdNum = parseInt(lineShapeObj.other["region_attributes"]["id"]);
			}
		   lineShapes.push(lineShapeObj);
		 }
	 }
...
function updatelabel(fileindex){
  ...
  for (var i=0;i<lineShapes.length;i++){
      lineShapes[i].type =  '';//regions[i].other.["region_attributes"]["type"];
	  var label = {'class_name':lineShapes[i].type, "score":1.0};
	  
	  if(!isEmpty(lineShapes[i].other.region_attributes.type)){
		  label['class_name'] = lineShapes[i].other.region_attributes.type;
	  }
	  if(!isEmpty(lineShapes[i].other.region_attributes.id)){
		  lineShapes[i].id = lineShapes[i].other.region_attributes.id;
	  }
	  var lineList = [];
	 
	  for(var j = 0; j < lineShapes[i].points.length; j++){
		  lineList.push(getRealLocationX(lineShapes[i].points[j].x));
		  lineList.push(getRealLocationY(lineShapes[i].points[j].y));
	  }
	  label['line'] = lineList;
	  label['id'] = lineShapes[i].id+'';
	  label['blurred']=lineShapes[i].blurred;
	  label['goodIllumination']=lineShapes[i].goodIllumination;
	  label['frontview']=lineShapes[i].frontview;
      label['other']=lineShapes[i].other;
      //console.log(label);
	  label_list.push(label);
  }
