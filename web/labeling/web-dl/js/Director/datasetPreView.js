...
if(!isEmpty(datasetInfo)){
	if(datasetInfo.datasetType == 4){
  ...
  else if (datasetInfo.datasetType == 3){//超大图像
      files  = ip+ datasetInfo.zip_object_name;
      console.log("files:",files);
      var filename = files.substring(files.lastIndexOf("/") + 1);
      ...
      showBigImg();
  }else if(datasetInfo.datasetType == 5){//文本预览
...
function isTextFile(){
	...
	console.log("posfix=" + labeltastresult[fileindex].pic_image_field.substring(length - 4))
...
function loadimg(){
	 var fname = labeltastresult[fileindex].pic_image_field.substring(labeltastresult[fileindex].pic_image_field.lastIndexOf('/') + 1);
	...
	 if(isTextFile()){
		canvas.width = 0;
        canvas.height = 0;
		console.log("text file,fname=" + fname);
		getTextContent(labeltastresult[fileindex].pic_image_field);
	    $("#textcontent").text(textContent);
	}else{
		console.log("picture file,fname=" + fname);
