...
public class MIAODSchedule {
	...
	private void doMIAODTrain(LabelTask labelTask) {
		...
		List<LabelTaskItem> itemList = labelTaskItemDao.queryLabelTaskItemByLabelTaskId(
				dbPolicy.getTableName(labelTask.getId(), UserConstants.LABEL_TASK_SINGLE_TABLE, labelTask.getUser_id()), labelTask.getId());
				//TokenManager.getUserTablePos(labelTask.getUser_id(), UserConstants.LABEL_TASK_SINGLE_TABLE), labelTask.getId());
		...
		if(labelTask.getTask_flow_type() == Constants.LABEL_TASK_FLOW_TYPE_MIAOD) {
			if(labelTask.getTotal_picture() >= 700) {//大于1000张
				...
				tmpParamMap.put("user_id",
						dbPolicy.getTableName(labelTask.getId(), UserConstants.LABEL_TASK_SINGLE_TABLE, labelTask.getUser_id()));
						//TokenManager.getUserTablePos(labelTask.getUser_id(), UserConstants.LABEL_TASK_SINGLE_TABLE));
