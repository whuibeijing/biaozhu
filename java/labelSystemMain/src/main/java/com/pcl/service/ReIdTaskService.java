...
public class ReIdTaskService {
	...
	public void addReIdTask(String token, ReIDTask body) throws LabelSystemException {
		...
		if(body.getLabelPropertyTaskId() != null) {
			...
			if(task != null) {
				body.setTask_label_type_info(task.getPropertyJson());
			}
		}
	...
	private int copyPredictTaskItem(int userId,ReIDTask body, String predictTaskId,int order) throws LabelSystemException {
		...
		List<PrePredictTaskResult> taskResultList = prePredictTaskResultDao.selectByPrePredictTaskId(
				dbPolicy.getTableName(predictTaskId, UserConstants.PREDICT_SINGLE_TABLE, predictTask.getUser_id()),predictTaskId);
				//TokenManager.getUserTablePos(predictTask.getUser_id(), UserConstants.PREDICT_SINGLE_TABLE),predictTaskId);
		...
		for(PrePredictTaskResult preTaskResult : taskResultList) {
			...
			if(batchList.size() == 2000) {
				...
				paramMap.put("user_id", dbPolicy.getTableName(body.getId(), UserConstants.REID_TASK_SINGLE_TABLE, predictTask.getUser_id()));
	...
	private int copyDateSetTaskItem(int userId, ReIDTask body, String dataSetId,int order) throws LabelSystemException {
		...
		List<LabelTaskItem> list = labelTaskItemDao.queryLabelTaskItemByLabelTaskId(
				dbPolicy.getTableName(dataSetId, UserConstants.LABEL_TASK_SINGLE_TABLE, userId),dataSetId);
				//TokenManager.getUserTablePos(userId, UserConstants.LABEL_TASK_SINGLE_TABLE),dataSetId);
		...
		for(LabelTaskItem item : list) {
			...
			if(batchList.size() == 2000) {//2000条执行一次保存
				Map<String,Object> paramMap = new HashMap<>();
				paramMap.put("user_id", dbPolicy.getTableName(body.getId(), UserConstants.REID_TASK_SINGLE_TABLE, userId));
						//TokenManager.getUserTablePos(userId, UserConstants.REID_TASK_SINGLE_TABLE));
				paramMap.put("list", batchList);
				reIdLabelTaskItemDao.addBatchLabelTaskItem(paramMap);
				batchList.clear();
			}
		...
		if(!batchList.isEmpty()) {
			Map<String,Object> paramMap = new HashMap<>();
			paramMap.put("user_id", dbPolicy.getTableName(body.getId(), UserConstants.REID_TASK_SINGLE_TABLE, userId));
					//okenManager.getUserTablePos(userId, UserConstants.REID_TASK_SINGLE_TABLE));
			paramMap.put("list", batchList);
			reIdLabelTaskItemDao.addBatchLabelTaskItem(paramMap);
		}else {
	...
	private void asyncDelete(String token, String reIDTaskId) {
		...
		reIdLabelTaskItemDao.deleteLabelTaskById(
				dbPolicy.getTableName(reIDTaskId, UserConstants.REID_TASK_SINGLE_TABLE, userId),reIDTaskId);
				//TokenManager.getUserTablePos(userId, UserConstants.REID_TASK_SINGLE_TABLE),reIDTaskId);
	...
	public void updateReIDInfo(String token, String reTaskId, String imageListInfo, String reId) {
		...
		for(Entry<String,Map<String,String>> entry : convertMap.entrySet()) {
			...
			LabelTaskItem item = reIdLabelTaskItemDao.queryLabelTaskItemById(
					dbPolicy.getTableName(reTaskId, UserConstants.REID_TASK_SINGLE_TABLE, userId),labelItemId);
					//TokenManager.getUserTablePos(userId, UserConstants.REID_TASK_SINGLE_TABLE),labelItemId);
	...
	public String upateAutoLabelInfo(Map<String, Object> msgMap,long progress) {
		...
		//这个地方的标注需要修改
		LabelTaskItem item = reIdLabelTaskItemDao.queryLabelTaskItemById(dbPolicy.getTableName(taskId, UserConstants.REID_TASK_SINGLE_TABLE, userId),itemId);
		...
		if(item.getLabel_status() == Constants.LABEL_TASK_STATUS_NOT_FINISHED || isEmpty(item.getLabel_info())) {
			...
			paramMap.put("user_id",dbPolicy.getTableName(taskId, UserConstants.REID_TASK_SINGLE_TABLE, userId)); 
	...
	private boolean isEmpty(String label_info) {
		if(!Strings.isNotBlank(label_info)) {
			return true;
		}
	...
	public int updateReIDLabelTaskItem(LabelTaskItem updateBody,int userId) {
		...
		LabelTaskItem oldLabelTaskItem = reIdLabelTaskItemDao.queryLabelTaskItemById(
				dbPolicy.getTableName(updateBody.getLabel_task_id(), UserConstants.REID_TASK_SINGLE_TABLE, userId),updateBody.getId());
				//TokenManager.getUserTablePos(userId, UserConstants.REID_TASK_SINGLE_TABLE),updateBody.getId());
		...
		paramMap.put("user_id", dbPolicy.getTableName(updateBody.getLabel_task_id(), UserConstants.REID_TASK_SINGLE_TABLE, userId));
				//TokenManager.getUserTablePos(userId, UserConstants.REID_TASK_SINGLE_TABLE));
	...
	public PageResult queryReIdTaskItemPageByLabelTaskId(String reIdTaskId,int currPage, int pageSize,String related_task_id,int orderType,int findLast,String token){
		...
		paramMap.put("user_id", 
				dbPolicy.getTableName(reIdTaskId, UserConstants.REID_TASK_SINGLE_TABLE, userId));
				//TokenManager.getUserTablePos(userId, UserConstants.REID_TASK_SINGLE_TABLE));
		...
		if(result != null) {
			...
			for(LabelTaskItem item : list) {
				...
				if(re) {
					...
					itemParamMap.put("item_add_time", TimeUtil.getCurrentTimeStr());
					itemParamMap.put("label_info", JsonUtil.toJson(labelList));
					itemParamMap.put("user_id", dbPolicy.getTableName(reIDTaskId, UserConstants.REID_TASK_SINGLE_TABLE, userId));
							//TokenManager.getUserTablePos(userId, UserConstants.REID_TASK_SINGLE_TABLE));
					reIdLabelTaskItemDao.updateLabelTaskItem(itemParamMap);
				}
			}
	...
	public void modifyAReIdImg(String token, String reIDTaskId, String reIdName, String imgName, String newReIdName) {
		...
		if(result != null) {
			...
			Map<String,Object> tmpParamMap = new HashMap<>();
			tmpParamMap.put("user_id", dbPolicy.getTableName(reIDTaskId, UserConstants.REID_TASK_SINGLE_TABLE, userId));
					//TokenManager.getUserTablePos(userId, UserConstants.REID_TASK_SINGLE_TABLE));
			tmpParamMap.put("list", itemIdList);
			List<LabelTaskItem> list = reIdLabelTaskItemDao.queryLabelTaskItemByIdList(tmpParamMap);
			//logger.info("list size=" + list.size());
	...
	public void deleteReIdLabel(String reIdTaskId, Integer startId, Integer endId,String one_reid_name, String token) {
		int userId = TokenManager.getUserIdByToken(TokenManager.getServerToken(token));
		List<LabelTaskItem> taskItem = reIdLabelTaskItemDao.queryLabelTaskItemByLabelTaskIdOderbyImageNameAsc(
				dbPolicy.getTableName(reIdTaskId, UserConstants.REID_TASK_SINGLE_TABLE, userId),reIdTaskId);
				//TokenManager.getUserTablePos(userId, UserConstants.REID_TASK_SINGLE_TABLE),reIdTaskId);
		...
		for(LabelTaskItem deleteItem : deleteItemList) {
			...
			paramMap.put("item_add_time", TimeUtil.getCurrentTimeStr());
			paramMap.put("user_id", dbPolicy.getTableName(reIdTaskId, UserConstants.REID_TASK_SINGLE_TABLE, userId));
					//TokenManager.getUserTablePos(userId, UserConstants.REID_TASK_SINGLE_TABLE));
			reIdLabelTaskItemDao.updateLabelTaskItem(paramMap);
		}
	...
	public int copyReIDLabelToOhterItem(String label_info, String destFileId, String token,String reIdTaskId) {
		...
		for(LabelTaskItem deleteItem : copyItemList) {
			...
			for(Map<String,Object> tmpMap : oldLabelList) {
				...
				oldLabelMap.put(id, tmpMap);
				int idInt = getIntId(id);
				if(idInt > maxId) {
					maxId = idInt;
				}
			}
			...
			for(Map<String,Object> tmpMap : newLabelList) {
				if(!oldLabelMap.containsKey(tmpMap.get("id").toString())) {
					...
					while(true) {
						maxId = maxId+1;
						if(!oldLabelMap.containsKey(String.valueOf(maxId))) {
							oldLabelList.add(LabelInfoUtil.putKey(tmpMap, "id", String.valueOf(maxId)));
							break;
						}
					}
				}
