...
public class AutoLabelService {
	...
	public void autoLabelTask(String token, AutoLabelBody body) throws LabelSystemException {
		...
		if(body.getTask_type() !=  null && body.getTask_type().equals(String.valueOf(Constants.DATASET_TYPE_PICTURE))) {
			...
			LabelTaskItem item = labelTaskItemDao.queryLabelTaskItemById(dbPolicy.getTableName(body.getTaskId(),UserConstants.LABEL_TASK_SINGLE_TABLE,userId),body.getLabel_task_id());
				...
				if(body.getModel() == WISE_MEDICAL_MODEL_ID) {
					ThreadSchedule.execThread(()->{
						try {
							...
							progressService.removeProgress(body.getTaskId());
			...
			if(isTrackingModel(body.getModel())){
				...
				ThreadSchedule.execThread(()->{
					...
					if(algId >= 10000) {
						trackingSchedule.doRemoteTrack(body,exceedTime);
					}else {
						try {
	...
	private boolean isTrackingModel(int model) {
		...
		if(model >= 10000) {
			AutoLabelModelRegistMsg autoModel = autoLabelModelRegistMsgDao.queryById(model);
			if(autoModel != null && (autoModel.getModel_type() == 5 || autoModel.getModel_type() == 6)) {
				return true;
			}
		}
		return false;
	...
	private void autoLabelCell(AutoLabelBody body,String filePath,String fileName,int userId) {
		...
		paramMap.put("user_id", dbPolicy.getTableName(body.getTaskId(),UserConstants.LABEL_TASK_SINGLE_TABLE,userId));
