...
public class PersonPropertyRetrain extends ATask{
	...
	private void copyLastModelToRightPath(RetrainTask retrainTask, String algRootPath,String saveDir) {
		...
		algModelDest.setModel_type(algModelSrc.getModel_type());
	...
	private void collectImageToDir(RetrainTask retrainTask,String trainDataSetPath,String valDataSetPath) {
		...
		for(String labelTaskId : labelTaskIdList) {
			List<LabelTaskItem> itemList = labelTaskItemDao.queryLabelTaskItemByLabelTaskId(
					dbPolicy.getTableName(labelTaskId, UserConstants.LABEL_TASK_SINGLE_TABLE, retrainTask.getUser_id()),labelTaskId);
					//TokenManager.getUserTablePos(retrainTask.getUser_id(), UserConstants.LABEL_TASK_SINGLE_TABLE),labelTaskId);
