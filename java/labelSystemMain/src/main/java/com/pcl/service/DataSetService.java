...
public class DataSetService {
	...
	private Progress getProgress(String id) {
		...
		return progressService.queryProgressById(id);
	...
	private void runChouZhen(DataSet dataSet,Map<String,String> para) {
		...
		try {
			...
			//删除已经存在的抽帧图片
			labelTaskItemDao.deleteLabelTaskById(dbPolicy.getTableName(dataSet.getId(), UserConstants.LABEL_TASK_SINGLE_TABLE, dataSet.getUser_id()),dataSet.getId());
			...
			for(File file : fileList) {
				...
				if(batchList.size() == 2000) {
					addBatchItemToDb(dataSet.getUser_id(),dataSet.getId(), batchList);
					batchList.clear();
				}
			}
			...
			progressService.removeProgress(dataSet.getId());
			if(saveTmpVideo == 1) {
				FileUtil.delDir(tmpPath);
			}
		}
		...
		ThreadSchedule.execThread(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					Map<String,String> fileMap  = fileService.unZipFileToMinio(downloadFile,bucketName);
					//删除下载的文件
					//downloadFile.delete();
					...
					for(Entry<String,String> fileEntry : fileMap.entrySet()) {
						String relativePath = "/dcm/" + bucketName +  "/" + fileEntry.getKey();
					...
					List<LabelTaskItem> batchList = new ArrayList<>();
					int count = 0;
					logger.info("start add to db, size=" + fileMap.size());
					for(Entry<String,String> fileEntry : fileMap.entrySet()) {
						LabelTaskItem taskItem = new LabelTaskItem();
						taskItem.setId(UUID.randomUUID().toString().replaceAll("-",""));
						taskItem.setItem_add_time(TimeUtil.getCurrentTimeStr());
						...
						count++;
						taskItem.setPic_real_name(fileEntry.getValue());
						...
						batchList.add(taskItem);
						if(batchList.size() == 2000) {
							logger.info("finished deal:" + count);
	...
	private void dealTextDataSet(DataSet dataSet) {
		Map<String,Object> paramMap = new HashMap<>();
		paramMap.put("id", dataSet.getId());
		ThreadSchedule.execThread(new Runnable() {
			@Override
			public void run() {
				...
				try {
					...
					for(Entry<String,String> fileEntry : fileMap.entrySet()) {
						...
						taskItem.setItem_add_time(TimeUtil.getCurrentTimeStr());
						taskItem.setLabel_task_id(dataSet.getId());
						String relativePath = "/minio/" + bucketName +  "/" + fileEntry.getKey();
						taskItem.setPic_image_field(relativePath);
						taskItem.setPic_real_name(fileEntry.getValue());
						taskItem.setLabel_status(Constants.LABEL_TASK_STATUS_NOT_FINISHED);
						batchList.add(taskItem);
					...
					paramMap.put("total", fileMap.size());
					paramMap.put("task_status", Constants.DATASET_PROCESS_PREPARE);
					paramMap.put("file_bucket_name", bucketName);
					dataSetDao.updateDataSet(paramMap);
					logger.info("upload dataset text status finised.");
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void addBatchItemToDb(int user_id,String datasetId,List<LabelTaskItem> batchList) {
		Map<String,Object> paramMap = new HashMap<>();
		logger.info("tableNamePrefix=" + dbPolicy.getTableName(datasetId,UserConstants.LABEL_TASK_SINGLE_TABLE,user_id));
		paramMap.put("user_id",dbPolicy.getTableName(datasetId,UserConstants.LABEL_TASK_SINGLE_TABLE,user_id));
				//TokenManager.getUserTablePos(user_id, UserConstants.LABEL_TASK_SINGLE_TABLE));
	...
	private void asyncDelete(String dataSetId, DataSet dataSet) {
		...
		if(dataSet != null) {
			...
			if(dataSet.getDataset_type() == Constants.DATASET_TYPE_DCM) {
				...
				List<LabelTaskItem> list = labelTaskItemDao.queryLabelTaskItemByLabelTaskId(dbPolicy.getTableName(dataSet.getId(), UserConstants.LABEL_TASK_SINGLE_TABLE, dataSet.getUser_id()),dataSetId);
						//TokenManager.getUserTablePos(dataSet.getUser_id(), UserConstants.LABEL_TASK_SINGLE_TABLE),dataSetId);
				labelTaskItemDao.deleteLabelTaskById(dbPolicy.getTableName(dataSet.getId(), UserConstants.LABEL_TASK_SINGLE_TABLE, dataSet.getUser_id()),dataSetId);
				for(LabelTaskItem item : list) {
					//删除磁盘上的文件
					new File(rootPath + item.getPic_image_field()).delete();
				}	
	...
	public PageResult queryDataSetPictureItemPage(String datasetId,int currPage,int pageSize){
		...
		paramMap.put("user_id", dbPolicy.getTableName(dataSet.getId(), UserConstants.LABEL_TASK_SINGLE_TABLE, dataSet.getUser_id()));
	...
	private void runVideoConcat(String datasetId, List<VideoInfo> videoList,String destFileName) {
		...
		try {
			...
			progressService.removeProgress(datasetId);
			FileUtil.delDir(tmpPath);
		}
	}
