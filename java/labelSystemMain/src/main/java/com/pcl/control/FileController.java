...
public class FileController {
	...
	@RequestMapping(value ="/download-obs-file", method = RequestMethod.POST)
	public void downloadOBSFile(@RequestParam("bucketName") String bucketName,@RequestParam("objectName") String objectName,@RequestParam("destPath") String destPath) {
		logger.info("download file :bucketName=" +  bucketName + " objectName=" + objectName + " destPath=" + destPath);
		try {
			fileService.downLoadFileFromMinio(bucketName, objectName, destPath);
	...
	@ResponseBody
	@ApiOperation(value="上传文件接口", notes="返回文件接口")
	@RequestMapping(value ="/unzip-file", method = RequestMethod.POST)
	public void unzipFile(@RequestParam("destPath") String destPath) {
		logger.info("unzip file : destPath=" + destPath);
		try {
			long fileSize=0;
			...
			try(ZipFile zip = new ZipFile(downloadFile,Charset.forName("GBK"))){//解决中文文件夹乱码
				for (Enumeration<? extends ZipEntry> entries = zip.entries(); entries.hasMoreElements();) {  
					...
					fileSize++;
					logger.info("zipEntryName=" + zipEntryName + " fileSize=" + fileSize);
				}  
			}
