...
public class PictureController {
	...
	@RequestMapping(method = RequestMethod.GET, value = "/api/gettextcontent")
	@ResponseBody
	public String getTextContent(@RequestParam(value="filename") String filename) {
		BucketObjectName bucketObjectName = new BucketObjectName(filename);
		logger.info("Get text area: " + filename);
		StringBuilder re = new StringBuilder();
		
		String code = "GBK";
		try(InputStream in= fileService.getPicture(bucketObjectName.getBucketName(), bucketObjectName.getObjectName())){
			 byte[] b = new byte[3];
		     in.read(b);
		...
		try(BufferedReader reader = new BufferedReader(new InputStreamReader(fileService.getPicture(bucketObjectName.getBucketName(), bucketObjectName.getObjectName()),code))){
			String line = null;
			while((line = reader.readLine()) != null) {
				if(code.equals("UTF-8")) {
					line = new String(line.getBytes(code),"GBK");
				}
				re.append(line);
			}
		} catch (Exception e) {
		...
		//logger.info("content: " + re.toString());
		return re.toString();
	}
