...
public class LabelInfoUtil {
	...
	public static Map<String,Object> putKey(Map<String,Object> label,String key,String value) {
		
		Map<String,Object> newLabel = new HashMap<>();
		newLabel.putAll(label);
		newLabel.put(key, value);
	...
	public static String getRealPath(String relativeFileUrl) {
		String tmps[] = relativeFileUrl.split("/");
		int find = -1;
		...
		if(find >= 0) {
			for(int i = find+1; i<tmps.length;i++) {
				str.append(tmps[i]).append("/");
			}
			str.deleteCharAt(str.length() - 1);
		}else {
			str.append(relativeFileUrl.substring(relativeFileUrl.lastIndexOf("/") +1));
		}
		return str.toString();
	}
}
