...
public class VocAnnotationsUtil {
			...
			for(Object obj : eleList) {
				...
				if(obj instanceof Element) {
					...
					while(iter.hasNext()) {
						...
						else if(tmpEle.getName().equals("line")) {
							String line = tmpEle.getText();
							String lines[] = line.split(",");
							List<Object> linepoints = new ArrayList<>();
							for(String lineEle : lines) {
								linepoints.add(lineEle);
							}
	...
	private Document produceDocment(Map<String, Object> typeOrColorMapName, ArrayList<Map<String, Object>> labelList,
		...
		for(Map<String,Object> label : labelList) {
			...
			Object lineObj = label.remove("line");
			if(lineObj != null) {
				Element lineEle = objEle.addElement("line");
				if(lineObj instanceof List) {
					lineEle.addText(getListStr((List<Object>)lineObj));
				}else {
					lineEle.addText(lineObj.toString());
				}
			}
