...
public class PclJsonAnnotationsUtil {
	...
	public Map<String,Object> getTextJson(LabelTaskItem item){
		...
		re.put("file_name", fileName);
		re.put("annotations", labelList);
	...
	public Map<String,Object> getJson(LabelTaskItem item,Map<String,Object> typeOrColorMapName){
		...
		for(Map<String,Object> label : labelList) {
			...
			if(boxList != null) {//矩形标注
				...
				if(lineList != null) {
					annotation.put("line", getListStr(lineList));
				}
			}
	...
	public LabelTaskItem readLabelInfoFromPclJson(InputStream in) {
		...
		return readLabelInfoFromJsonMap(map);
	}

	public LabelTaskItem readLabelInfoFromJsonMap(Map<String, Object> map) {
		...
		if(map.get("bbox") != null && map.get("bbox") instanceof List) {
			...
			for(Map<String,Object> objMap : annotationList) {
				...
				Object lineObj = objMap.remove("line");
				if(lineObj != null) {
					String seg = lineObj.toString();
					String segs[] = seg.split(",");
					List<Object> linepoints = new ArrayList<>();
					for(String segEle : segs) {
						linepoints.add(segEle);
					}
					label.put("line", linepoints);
				}
